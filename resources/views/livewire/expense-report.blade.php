...
<div class="p-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div
            class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg p-4 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-red-600 dark:text-red-400">RS. {{
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 border-l-4 border-green-500">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-green-600 dark:text-green-400">RS. {{
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 overflow-hidden">
            ...
            <div class="relative flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p
                        class="text-2xl font-semibold {{ ($summary['net_balance'] ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 mb-6 ring-1 ring-gray-200 dark:ring-gray-700">
        <h2 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100"> Net Balances ({{
            $this->getPeriodDescription($this->filters) }})</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        ...
                        <td
                            class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium {{ $balance['is_positive'] ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            @if($balance['net_balance'] > 0)
                            {{ $person }} owes you RS. {{ number_format($balance['net_balance'], 2) }}
                            @elseif($balance['net_balance'] < 0) You owe {{ $person }} RS. {{
                                number_format(abs($balance['net_balance']), 2) }} @else All settled @endif </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-48 flex-shrink-0">
            <div
                class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sticky top-4 ring-1 ring-gray-200 dark:ring-gray-700">
        ...
        <div
            class="flex-1 bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden ring-1 ring-gray-200 dark:ring-gray-700">
            <div class="overflow-x-auto">
                <table id="debtsTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr
                            class="{{ $debt->is_settled ? 'opacity-70' : '' }} hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{
                                Carbon::parse($debt->expense_date)->format('Y-m-d') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400">You
                                    Owe</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $person
                                }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{
                                $debt->expense->description }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-red-600 dark:text-red-400">-
                                RS. {{ number_format($debt->amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $debt->is_settled ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400' }}">
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400">Owes
                                    You</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $person
                                }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{
                                $debt->expense->description }}</td>
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm text-right text-green-600 dark:text-green-400">
                                + RS. {{ number_format($debt->amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $debt->is_settled ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400' }}">
